...
{
    int i;
    for(i = 0; i < graph->size && memcmp(&graph->vertices[i], pos, sizeof *pos) != 0; i++); // A REVOIR !!!!!!!!!!!!!!!!!!!
    assert(i < graph->size);
...
{
    size_t i;
    for(i = 0; i < graph->size && memcmp(&graph->vertices[i].pos, src, sizeof *src) != 0; i++) // Supprimer i < graph->size
