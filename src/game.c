...
const position *nextAction(gameGraph *graph, const position *catPos, const gameMap *map) {
  ...
  if (graph->vertices[i].dist > -1)
    ...
    return NULL;
  while (memcmp(&min->reachedFrom->pos, catPos, sizeof *catPos) != 0) min = min->reachedFrom;
  return &min->pos;
}

bool neighbors(const position *p1, const position *p2) {
  if (p1->i == p2->i && (p1->j + 1 == p2->j || p1->j == p2->j + 1)) return true;
  if (p1->j == p2->j && (p1->i + 1 == p2->i || p1->i == p2->i + 1)) return true;
