...
static void play()
{
    ...
    xStart = X_START + (X_END - X_START - (X_OFFSET*(map->size-1) + unmarkedpi->rect.w*(map->size)))/2;
    yStart = Y_START + (Y_END - Y_START - (Y_OFFSET*(map->size-1) + unmarkedpi->rect.h*(map->size)))/2;
    ...
    while(true){
        ...
        while(true){
            ...
            if(e.type == SDL_QUIT)
                ...
                if(exist && map->mat[p.i][p.j] == UNMARKED && memcmp(&p, &catPos, sizeof p) != 0)
                    break;
            }
        }
